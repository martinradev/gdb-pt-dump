...
def create_resources(arch_name, linux_image, kaslr):
    ...
    old_monitor = QemuOldMonitorExecutor(vm)
    memory_flatview = old_monitor.get_memory_flat_view()

    if bool(os.getenv("GDB_PT_DUMP_TESTS_PAUSE_AFTER_BOOT")) == True:
        print("Sleeping...")
        time.sleep(10000)

    return (vm, gdb, monitor, old_monitor, memory_flatview)

@pytest.fixture
def create_resources_fixture_nokaslr(arch_name, linux_image):
    vm, gdb, monitor, old_monitor, memory_flatview = create_resources(arch_name, linux_image, kaslr=False)
    yield (vm, gdb, monitor, old_monitor, memory_flatview)
    monitor.stop()
    gdb.stop()
    old_monitor.stop()
    vm.stop()
...
def create_resources_fixture(arch_name, linux_image):
    vm, gdb, monitor, old_monitor, memory_flatview = create_resources(arch_name, linux_image, kaslr=True)
    yield (vm, gdb, monitor, old_monitor, memory_flatview)
    monitor.stop()
    gdb.stop()
    old_monitor.stop()
...
def test_pt_smoke(create_resources_fixture, arch_name, linux_image):
    vm, gdb, monitor, old_monitor, memory_flatview = create_resources_fixture
    res = gdb.run_cmd("pt")
...
def test_pt_search_string(create_resources_fixture, arch_name, linux_image):
    ...
    _test_pt_search(vm, gdb, old_monitor, "pt -ss Linux", 5, checker)

@pytest.mark.parametrize("arch_name, linux_image", get_all_images())
def test_pt_search_s4(create_resources_fixture, arch_name, linux_image):
    vm, gdb, monitor, old_monitor, memory_flatview = create_resources_fixture
    checker = lambda _, mem: mem == b"\x41\x41\x41\x41"
    _test_pt_search(vm, gdb, old_monitor, "pt -s4 0x41414141", 4, checker)
...
def test_pt_search_s8(create_resources_fixture, arch_name, linux_image):
    ...
    _test_pt_search(vm, gdb, old_monitor, "pt -s8 0xfffffffffffffffe", 8, checker)
...
def test_pt_range_exists(create_resources_fixture, arch_name, linux_image):
    ...
    assert(len(ranges) > 0)
    for r in ranges:
        if check_if_belongs_to_io_or_rom(old_monitor, memory_flatview, r.va_start):
            print(f"Skipping {str(r)}")
            continue

        print("Range base is", hex(r.va_start))
        addr = r.va_start
        assert(check_va_exists(old_monitor, monitor, memory_flatview, addr))
...
def test_pt_filter_executable(create_resources_fixture, arch_name, linux_image):
    ...
    _test_pt_filter_common(vm, gdb, old_monitor, executions)
...
def test_pt_filter_and_search(create_resources_fixture, arch_name, linux_image):
    ...
    vm, gdb, monitor, old_monitor, memory_flatview = create_resources_fixture
    _test_pt_search(vm, gdb, old_monitor, "pt -ss Linux -filter w", 5, checker)
    ...
    checker = lambda occ, data: \
        ...
        data == b"Linux"
    _test_pt_search(vm, gdb, old_monitor, "pt -ss Linux -filter ro", 5, checker)
...
def test_pt_kaslr(create_resources_fixture_nokaslr, arch_name, linux_image):
    ...
    vm, gdb, monitor, old_monitor, memory_flatview = create_resources_fixture_nokaslr
...
def test_golden_images(request, create_custom_resources_fixture, arch_name, image_name):
    vm, gdb, monitor, old_monitor, flatview = create_custom_resources_fixture
...
def test_pt_x86_32():
    ...
    for r in ranges:
        addr = r.va_start
        data = old_monitor.read_virt_memory(addr, 4)
        assert(len(data) == 4)
...
def test_pt_i386():
    ...
    monitor = QemuOldMonitorExecutor(vm)
